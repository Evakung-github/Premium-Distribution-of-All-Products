...
workbook.save('D:\eva\python\保費分布\output2.xlsx')
...
for i in range(len(par)):
    ...
    try:
        ...
        q = d.pivot_table(index='sex', columns='discount', values='pl_prem',margins = True,aggfunc = func,fill_value = 0)
        q = DataFrame(np.array([q.iloc[-1],q.iloc[1].div(q.iloc[-1]),q.iloc[0].div(q.iloc[-1])],dtype = float),
        ...
        d = DataFrame(table)
        r = d.pivot(index='key_age', columns='sa_index', values='ratio')
